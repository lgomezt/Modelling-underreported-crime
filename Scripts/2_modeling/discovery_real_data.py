...
class Chen:
    ...
    def __init__(self, M, K, N, report, preport) -> None:
        self.M = M 
        self.N = N
        self.K = K
    ...
    def update_mu_hat_t_i(self, S, X_T, t, S_T):
        ...
        for i in range(self.M):
            if bool(S[i]):
                x[i] = self.report[i][t - 1] / self.N
            else:
                x[i] = self.preport[i][t - 1] / self.N
...
class UCB1:
    ...
    def initialization(self, T_i):
        ...
        x = [
            np.random.choice(self.report[i]) / self.N + 1e-11\
                for i in range(self.M)
        ]
...
class LLR:
    def __init__(self, M, K, N, report, preport, underreporting=False) -> None:
        self.M = M
        self.K = K
